...
class Fighter():
    ...
    def move(self, screen_width, screen_height, surface, target, round_over):
        ...
        if self.attacking == False and self.alive == True and round_over == False:
            ...
            # check player 2 controls (bot)
            elif self.player == 2 and not round_over:
                # Bot's behavior
                ...
                # Jump to prevent attacks
                if target.attacking and random.choice([True, False]):
                    if not self.jump:
                        self.vel_y = -30
                        self.jump = True
    ...
    def attack(self, target):
        if self.attack_cooldown == 0:
            ...
            if attacking_rect.colliderect(target.rect):
                # Player 1 (user) deals 10 damage
                if self.player == 1:
                    target.health -= 6
                # Player 2 (bot) deals 7 damage
                elif self.player == 2:
                    target.health -= 6
                target.hit = True
